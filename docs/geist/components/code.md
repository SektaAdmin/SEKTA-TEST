...
> Автозгенеровано з `Vercel_raw/geist-docs/vercel.com/geist/code.html` скриптом `scripts/geist/extract-components.mjs` (`npm run geist:components`). Не редагувати вручну.
...
```
import { Snippet } from '@vercel/geistcn/components'; import type { JSX } from 'react'; export function Component(): JSX.Element { return <Snippet text="npm init next-app" width="300px" />; }
```
