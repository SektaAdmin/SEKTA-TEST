...
- [x] **Сесія 0 — Оцифровка Geist у токени.** Нейтралі теплі→холодні Geist: `--bg #f5f5f2→#fafafa`, `--bg-3 #eeede9→#f2f2f2`, `--text #1a1917→#171717`, `--accent-hover →#171717`; база радіуса `--radius 10px→8px` (Geist md). Семантичні кольори й brand-шкала (вже ахроматична) без змін. Синхронено: inline-bg `app/layout.tsx`, коментар `client.module.css`, `DESIGN.md` (frontmatter + проза). Шрифт Geist уже стояв (f1a8dc4). Гілка `redesign/session-0-tokens`.
