...
- [x] **Сесія 1 — Кнопки + поля.** Система кнопок у `globals.css`: `.btn-primary` + `.btn-secondary`/`.btn-ghost`/`.btn-danger` (кожна самодостатня, одна класа) + модифікатор `.btn-sm`. Спільна база: `--control-h`, `--radius-sm`, 14/500, `1px` transparent-рамка, `--motion-fast`, `:disabled` opacity .5. Ghost = look «Скасувати» (transparent + `--border-hover` + `--text-2`, hover fill). `ModalFooter` переведено на глобальні класи (cancel→ghost, save→primary, danger→danger); `ModalFooter.module.css` видалено. Поля (`FormField`): padding 8/12, focus-glow `0 0 0 3px rgba(0,0,0,.08)`, `aria-invalid` → червона рамка + `--danger-dim` glow (FormField ставить прапорець при error), errorHint → `--danger-text` (контраст 4.5:1 на 11px), disabled → `--border-strong` + `--text-3`. Хардкоди кнопок по сторінках НЕ мігровано — це робота Сесій 4–11. `DESIGN.md` §Buttons/Inputs звірено з реалізацією. Гілка `redesign/session-1-buttons-fields`.
