...
| `salary_payment_reconcile` | фінальні виплати, де live-пересчёт (`calc_trainer_salary_v2` за period_start/end, межі `T00:00:00`/`T23:59:59` UTC як в UI) ≠ снапшоту `calculated_amount`. security_invoker → бачить лише owner | `listSalaryPaymentDrifts` → danger-бейдж «live N ₴» |
...
- `trainer_payments` — EXCLUDE `trainer_payments_final_no_overlap` (gist): періоди `payment_type='final'` одного тренера НЕ перекриваються (перекриття задвоювало «До виплати»). Аванси вільні — легітимно перекриваються з final за той самий період. 23P01 → UI-помилка (`paymentErrorMessage`). Снапшот `calculated_amount` = «Нараховано» на момент фіксації; звірка з live → view `salary_payment_reconcile`.
...
| `generate_week(p_start_date, p_weeks=1)` | → `(classes_created, enrollments_created, conflicts jsonb)`. Ідемпотентна (UNIQUE `uq_classes_series_date`). Прокидує series_clients в enrollments. Слот із перетином по залу/тренеру (`check_class_conflicts`) НЕ створюється → елемент у `conflicts` (series_title, class_date, time_of_day, hall_name, conflict_type, conflict_with_title…), UI показує warning-тост. Повторний запуск досоздає відсутні enrollments у існуючі НЕскасовані заняття серії (DO NOTHING — вручну скасовані записи не реанімує); скасоване заняття серії блокує пересоздання. Ставить `app.suppress_enroll_notify='on'` (local) → масові enrollments лог-яться в `enrollment_events`, але БЕЗ Telegram-доставки (щоб не лити десятки нотифікацій власнику). frontend `generateWeek()` |
...
| `calc_trainer_salary_v2(p_trainer_id, p_start, p_end)` | рядок на enrollment: attended+noshow+`cancelled` із `sessions_used>0` (пізня відміна). Інваріант: клієнту списали сесію → тренеру оплата `rate × e.sessions_used`. Рання відміна/скасоване заняття (`sessions_used=0`) — ні. Ставка з `trainer_rates` на дату заняття; нема ставки → 0 + `rate_missing=true` (UI: warning-баннер + підсвітка). EXECUTE лише `authenticated`; зміна RETURNS TABLE = DROP+CREATE → гранти вручну. Для `/settings/salary/calculations` |
...
| `render_enrollment_event_message(p_event_type, p_class_id, p_client_id, p_actor_role, p_enrollment_id) → text` | SECURITY DEFINER STABLE. Українське повідомлення для Telegram, час `Europe/Kyiv`. Викликає `emit_enrollment_event` для notify-рядків. EXECUTE лише `postgres`; легасі 4-арг overload видалено (2026-07-07) |
