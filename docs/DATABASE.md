...
| `calc_trainer_salary_v2(p_trainer_id, p_start, p_end)` | рядок на enrollment: attended+noshow+`cancelled` із `sessions_used>0` (пізня відміна). Інваріант: клієнту списали сесію → тренеру оплата `rate × e.sessions_used`. Рання відміна/скасоване заняття (`sessions_used=0`) — ні. Ставка з `trainer_rates` на дату заняття; нема ставки → 0 (без помилки). Для `/settings/salary/calculations` |
...
| `accounting_balance(p_method, p_holder=null)` | → `TABLE(income int, outcome int, balance int)`. SECURITY DEFINER (`search_path`), STABLE, EXECUTE `authenticated`. Повний баланс рахунку (метод+опц. cash_holder) за ВСЮ історію одним SUM в БД — дзеркало клієнтської логіки: sale income=`ticket_id IS NOT NULL ? price_paid : greatest(0,amount_given)`, expense `direction`, payment `round(paid_amount)`. Щоденна звірка ФОП/готівки на /accounting. frontend `getAccountingBalance()` |
