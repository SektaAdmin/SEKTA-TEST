> ⚠️ **LEGACY (заморожено, не редагувати).** Передрольовий знімок (останнє оновлення 2026-05-07).
> Описує модель «authenticated = повний доступ», якої **більше немає** — її замінила рольова модель
> (owner/admin/trainer/client) у [../SECURITY.md](../SECURITY.md). Актуальне джерело правди — кореневий
> `CLAUDE.md`. Лишено для історії; зміст не переписується.
...
0. **AGENTS.md завжди актуальний** — після кожної задачі що змінює архітектуру, компоненти, хуки або DB — оновлювати AGENTS.md в тому ж коміті.
