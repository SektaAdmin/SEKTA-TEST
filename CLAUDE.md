...
| `/client` | **Кабінет клієнта** (mobile-first; `layout.tsx` = `.shell`, шапка зверху + `.scroll` всередині, бо на мобільному html/body `overflow:hidden`). Зверху вниз: **hero «наступне заняття»** (найближчий активний запис `starts_at > now`, скасовані виключені; кнопка self-відміни) → **баланс** (вертикальний СПИСОК, не карусель: депозит окремим акцентним рядком над заняттями по типах, повна назва з `training_types.label`) → **вкладки** Майбутні/Архів/Покупки (`useState`): майбутні (hero-запис відфільтровано, відносні дати «Сьогодні»/«Завтра» через `relativeDay`, waitlist-бейдж) + self-відміна (`client_cancel`); архів (минулі attended/noshow/cancelled, бейдж статусу); покупки (усі sales: з тікетом = абонемент; `ticket_id=null` = поповнення `+amount_given`/списання `−price_paid` депозиту; snapshot інв. #5, БЕЗ balance_transactions) → **профіль read-only внизу** (лише контакти, імʼя — у шапці; пометка «зверніться до адміністратора»; контакти лише SELECT за RLS). Запис на нові заняття — наступна ітерація (бекенд `client_enroll` готовий). Зона client |
