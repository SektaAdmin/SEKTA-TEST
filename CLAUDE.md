...
- **⚠️ Embed `trainers(...)` на `sales`/`studio_expenses`/`trainer_payments` — ТІЛЬКИ через явний FK із `TRAINER_FK` (`lib/queries/_fk.ts`).** Ці три таблиці мають ДВА FK на `trainers` (`trainer_id` + `cash_holder`), тому голий `trainers(name)` → рантайм «Could not embed because more than one relationship was found». Писати `trainers!${TRAINER_FK.sales}(name)` (`.expenses`/`.payments`). Решта таблиць (classes/enrollments/trainer_rates/class_series) мають один FK → голий `trainers(name)` ок. Перевірка: `grep -rn "trainers(" lib/queries/{sales,studio-expenses,accounting,trainer-rates}.ts | grep -v "_fkey\|_fk.ts" | grep "trainers("` (двофкові embed) → лише `TRAINER_FK`-форми.
