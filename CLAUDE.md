...
- **⚠️ Серверні Route Handlers зі service-role — лише в `app/api/**`.** Створення `auth.users` (логіни клієнтів) потребує `SUPABASE_SERVICE_ROLE_KEY` (`createClient<Database>(url, serviceKey)` + `auth.admin.createUser`), який НЕ можна світити в браузер → robиться у Route Handler, не в `lib/queries`/компоненті. Єдиний на зараз: `app/api/admin/create-client-login/route.ts` (Фаза 5) — гейтить `isStaff` через `getRole()`, читає телефон з `client_contacts`, нормалізує через `normalize_phone_ua()`, перевіряє `clients.user_id IS NULL` (UNIQUE-констрейнт `clients_user_id_key`) + що номер не зайнятий, створює auth-юзера з `app_metadata.role='client'`, привʼязує `clients.user_id`. Identifier = телефон (E.164 `+380…`); зараз логін+пароль (адмін шле в директ), далі — OTP по SMS на той самий `auth.users.phone`. middleware теж відсікає не-staff на `/api/**` (редирект). Виняток з інваріанта «всі `.from()`/`.rpc()` у `lib/queries`»: admin-клієнт у Route Handler пише напряму.
