...
| `/client` | **Кабінет клієнта** (mobile-first; `layout.tsx` = `.shell`, шапка зверху + `.scroll` всередині, бо на мобільному html/body `overflow:hidden`). Зверху вниз: **профіль read-only** (лише контакти, імʼя — у шапці; пометка «зверніться до адміністратора»; контакти лише SELECT за RLS) → **карусель балансів** (гориз. свайп `flex:0 0 85%`+scroll-snap+точки: депозит + картка на кожен тип занять, повна назва з `training_types.label`) → **вкладки** Майбутні/Архів/Покупки (`useState`): майбутні + self-відміна (`client_cancel`); архів (минулі attended/noshow/cancelled, бейдж статусу); покупки (усі sales: з тікетом = абонемент; `ticket_id=null` = поповнення `+amount_given`/списання `−price_paid` депозиту; snapshot інв. #5, БЕЗ balance_transactions). Запис на нові заняття — наступна ітерація. Зона client |
