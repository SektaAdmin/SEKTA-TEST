...
- **`enrollments.status`** — `enrolled` / `attended` / `cancelled` / `noshow` / `waitlist`. Тригер `check_class_capacity` авто-переводить `enrolled`→`waitlist` при повному залі. **Фінансовий факт — у `sessions_used`** (>0 = сесію списано), не в окремому статусі: `cancelled` зі `sessions_used>0` = «скасовано пізно, штраф». Свідомо НЕ розділяємо статус на early/late (єдине джерело правди зі `sessions_used`).
- **`enrollments.cancellation_source`** — хто скасував: `self` (клієнт) / `staff_manual` (персонал, дефолт у `change_enrollment_status`/`reverse_attendance`) / `class_cancelled` (заняття скасовано студією) / `auto_close`. `enrollments.cancelled_at` — час переходу в `cancelled` (NULL поки не скасовано; RPC ставлять `now()` при `cancelled`, скидають у NULL при виході). `enrollments.cancelled_from_status` — з якого статусу скасовано (для `cancel_class_and_restore_sessions`). «Рано/пізно» — похідне від `sessions_used`, НЕ окреме поле.
...
| `mark_attendance(p_enrollment_id, p_sessions_used=1)` | → `(success, error_message)`. Декремент сесій (allow negative — балансу нема → йде в мінус), status=attended. **Вживає лише cron**; UI → `change_enrollment_status`. **Гейт `can_manage_enrollment()`** (див. нижче); EXECUTE лише `postgres` (з PostgREST недоступна) |
| `change_enrollment_status(p_enrollment_id, p_new_status, p_force_no_charge=false, p_sessions_used=null)` | → `(success, charged, error_message)`. Єдина точка зміни статусу з UI. Вирівнює баланс сесій + застосовує правило скасування. `charged` = чи списано сесію. Ставить `cancelled_at=now()` + `cancellation_source='staff_manual'` при `cancelled`, скидає в NULL при виході з cancelled. **Гейт `can_manage_enrollment()`** (див. нижче) |
| `delete_enrollment(p_enrollment_id)` | → `(success, error_message)`. **Фізичне** видалення помилкового запису (на відміну від `cancelled` — лишається в журналі). Повертає `sessions_used` у баланс сесій ПЕРЕД `DELETE`. **Свідомий виняток з інваріанта #7 (м'які видалення)**: «випадкового» запису не повинно існувати в історії взагалі (напр. записали не того). Не плутати зі скасуванням. frontend — `deleteEnrollment()` + опція «Видалити запис» у `ClassDetailModal` (inline-підтвердження). **Гейт `can_manage_enrollment()`** |
...
| `cancel_class_and_restore_sessions(p_class_id)` | → `(success, restored_count, error_message)`. attended→повернути sessions_used; noshow→duration/60; enrolled→скасувати без повернення; is_cancelled=true. **Гейт `can_manage_enrollment()`** |
| `delete_class(p_class_id)` | → `(success, restored_count, error_message)`. **Фізичне** видалення помилково створеного заняття (на відміну від `cancel_class_and_restore_sessions` — м'яке, лишається в історії). Повертає всі списані сесії (агрегат по client_id) ПЕРЕД `DELETE classes`; записи зносить CASCADE (`enrollments_class_id_fkey`). `restored_count` = к-сть клієнтів, кому повернули. **Свідомий виняток з інв. #7.** ⚠️ заняття з `series_id` (постійний шаблон) `generate_week` пересоздасть при повторному «виставити тиждень» (UI попереджає). frontend — `deleteClass()` + кнопка «Видалити» у `ClassDetailModal`. **Гейт `can_manage_enrollment()`** |
| `can_manage_enrollment() → boolean` | Гейт привілейованих enrollment-RPC (ці 6 — `SECURITY DEFINER`, оминають RLS: `change_enrollment_status`/`mark_attendance`/`cancel_class_and_restore_sessions`/`reverse_attendance`/`delete_enrollment`/`delete_class`). `true` якщо: owner/admin (staff-UI з браузера, роль `authenticated`) **АБО** session-флаг `app.trusted_call='on'` (довірений внутрішній виклик — `client_cancel` ставить його перед делегуванням) **АБО** немає JWT І `current_user∉{anon,authenticated}` (cron під роллю `postgres`). Інакше залогінений client/trainer і anon → відмова. **Завжди boolean** (COALESCE — інакше `NOT NULL=NULL` обходить гейт). EXECUTE на ці RPC видано лише `authenticated`+`postgres` (НЕ PUBLIC/anon) |
...
| `update_training_type_sort_orders(p_ids[], p_orders[])` | → `void`. Пакетний `UPDATE training_types.sort_order` (перетягування порядку довідника). SECURITY DEFINER (оминає RLS) → **гейт усередині `auth_role() IN ('owner','admin')`** (RAISE EXCEPTION інакше), `SET search_path`. EXECUTE відкликано від PUBLIC, видано лише `authenticated`. frontend — `reorderTrainingTypes()` |
...
| `client_enroll(p_class_id)` | → `(success, enrollment_id, enrolled_status, error_message)`. Клієнтський self-запис із кабінету. Гейт: роль=client, є оплачені заняття типу (`error='no_sessions'`), без конфлікту (`'conflict'`)/дубля (`'duplicate'`), **не в мінус**. **Справедливість черги (лише self-запис):** RPC сам обчислює статус — у `waitlist`, якщо місць немає (`active >= capacity`) **АБО в черзі вже хтось є** (новий не перестрибує тих, хто чекає); інакше `enrolled`. Вставляє явно обчислений статус; тригер `check_class_capacity` лишається запобіжником (спрацьовує лише на `enrolled`, тільки понижує). **Адмінський флоу/тригер для інших записів НЕ змінено** — правило черги тільки тут. `enrolled_status` = підсумковий статус (кабінет показує точний бейдж/тост). Авто-промоушн із черги при відміні НЕ робимо (адмін вручну). **Двогодинне заняття (`duration_min>=120`) → проставляє `hours_attended=[1,2]`** (клієнт бере все заняття) → `auto_close` спише 2 сесії, а не 1 (NULL→1). Списання — потім, через auto_close |
| `get_session_balances_running(p_client_id, p_from)` | → `TABLE(enrollment_id, balance_after)`. SECURITY DEFINER (`search_path` set), EXECUTE лише `authenticated`, client-гейт (лише себе). **Наростаючий** залишок сесій ПІСЛЯ кожного майбутнього запису ОДНОГО клієнта по всіх типах (window `SUM(cost) OVER PARTITION BY ticket_type ORDER BY starts_at`; cost=`COALESCE(array_length(hours_attended,1),1)`, is_cancelled→0). Рахує по ВСІХ майбутніх записах (не по slice) → пагінація UI не ламає кумулятив. Замінив клієнтський цикл у ClientVisits. Не плутати з точковим `get_session_balance_after` (масив клієнтів × 1 заняття, для адмінки). frontend — `listMyRunningBalances()` |
| `class_availability(p_class_ids[])` | → `TABLE(class_id, active_count, waitlist_count, capacity)`. SECURITY DEFINER, лише **числові агрегати** (без PII → IDOR-безпечно), бо client не бачить чужих enrollments (RLS `client_select_own`). Дає кабінету заздалегідь знати, що запис піде в резерв (текст кнопки «Записатись у резерв»). EXECUTE лише `authenticated`. frontend — `getClassAvailability()`; дзеркало логіки черги — `goesToWaitlist()` у `lib/scheduleMetrics.ts` |
| `client_cancel(p_enrollment_id)` | → `(success, charged, error_message)`. Клієнтська self-відміна. Перевіряє «це мій запис», ставить `app.trusted_call='on'` (щоб пройти гейт `can_manage_enrollment`), делегує в `change_enrollment_status('cancelled')`. `p_force_no_charge` НЕдоступний (зашито `false`) |
| `auto_close_classes()` | pg_cron щохвилини. Модель «почалось = проведено»: закриває всі `enrolled` для занять із `starts_at <= now()` (без верхньої межі) через `mark_attendance` → `attended`. **Списує `COALESCE(array_length(hours_attended,1), 1)` сесій** — поважає чекбокс годин, проставлений адміном заздалегідь при записі (двогодинне з `[1,2]` → 2 сесії; `[1]`/`[2]` → 1; годинне `NULL` → 1); в мінус якщо нема. Непришедших адмін переводить у `noshow`/`cancelled` вручну постфактум. **Запис постфактум** (`enrollClient` у вже-минуле заняття) закривається одразу в `attended` через `change_enrollment_status` (НЕ `mark_attendance` — та має EXECUTE лише для `postgres`, з браузера PostgREST її відхиляє → запис лишався б enrolled до тику cron), не чекаючи тик cron (cron лишається страховкою) |
...
- **Лейбли+класи бейджів** (статуси enrollment, методи оплати, короткі типи, типи balance-транзакцій) → `lib/badges.ts`. `enrollmentStatusClass`/`paymentClass` повертають готовий `'badge badge-cash'` → у `className` напряму. CSS бейджів — у `globals.css`. Лейбли статусів — **безособові, без роду, єдиний тон в адмінці і кабінеті** (Заброньовано/Відвідано/Не відвідано/Скасовано/У резерві — як «Клієнт записаний» в Altegio, НЕ «Записалась»). `personal_card` = «Картка». `transactionTypeLabel(type)` — людська назва `balance_transactions.transaction_type` (purchase/deposit_topup/admin_adjustment + історичні deduction/refund/adjustment).
- **Зведений бейдж запису** (єдина точка розшифровки скасування) → `enrollmentBadge({status, cancellation_source, sessions_used}, 'admin'|'client') → {label, tone}` у `lib/badges.ts`. Текст статусу єдиний; різниця перспектив лише в ДЖЕРЕЛІ cancelled: admin — суфікс «· студія/адмін/клієнт/авто» (через `cancelSourceSuffix`), client — без джерела. «Пізно» (`sessions_used>0`) → «Скасовано (пізно)». `tone` (`enrolled/attended/cancelled/late/noshow/waitlist`) місце мапить на клас: глобальний `badge badge-*` через `enrollmentBadgeClass(tone)` (admin) або власний CSS-Module (кабінет — `VISIT_TONE_CLASS` у ClientVisits). НЕ зливати з `cancellation.ts` (факт vs прогноз).
...
- **Дати display** → `lib/formatters.ts`: `formatDate` (ДД.ММ.РРРР), `formatDateShort` (ДД.ММ), `formatDateYY` — вхід ISO/Date (через `new Date`); `hhmm(Date)` (ГГ:ХХ), `fullWhen(startISO, durationMin)` («Пʼятниця, 5 червня, 14:00 – 15:00», вживає кабінет), `pluralHours(n)` (відмінювання «година/години/годин» — залишок занять рахується годинами). Дата → РРРР-ММ-ДД для `<input type=date>` → `toYMD`/`isoToYMD` у `lib/dateUtils.ts`. ⚠️ Для РРРР-ММ-ДД-рядка (значення date-picker'ів) display і parse — split-based у `lib/dateUtils.ts`: `ymdToDisplay` (→ДД.ММ.РРРР) і `parseYMD` (→Date local-midnight), БЕЗ `new Date(ymd)` (TZ-ризик). Не писати `getFullYear()+padStart` локально.
- **Дедлайн безкоштовного скасування** → `cancellationDeadline(startISO)` / `isFreeCancellation(startISO, nowMs?)` у `lib/cancellation.ts` — клієнтська копія БД `cancellation_deadline` (RPC), прив'язана до **Europe/Kyiv** (через `Intl`, не до таймзони пристрою — БД рахує в київському). Дозволяє показати правило відміни в модалці кабінету ДО виклику `client_cancel`. Логіка мусить лишатися синхронною з БД-функцією.
...
- **Метрики розкладу** (capacity/waitlist/fill) → `lib/scheduleMetrics.ts` (`getActiveCount`, `getWaitlistCount`, `isFull`, `isAlmost`, `fillPct`; для шаблонів — `*ClientCount*`; `goesToWaitlist(availability)` — дзеркало логіки RPC `client_enroll`: місць нема АБО черга вже є → waitlist). Не дублювати формули.
...
Ще НЕ централізовано (чекає на друге місце перед виносом): —.
...
| `/client`, `/client/schedule`, `/client/visits`, `/client/visits/[id]`, `/client/subscriptions` | **Кабінет клієнта** — меню-лаунчер + окремі екрани (стиль Altegio), mobile-first (`layout.tsx` = `.shell` + `.scroll`, бо html/body `overflow:hidden` на моб.). **`/client`** = головна-меню (`ClientHome`): шапка SEKTA + адреса (`STUDIO`) → профіль-картка read-only (аватарка-ініціал + імʼя + email/телефон, «зверніться до адміністрації») → меню (Розклад / Мої візити / Абонементи / Вийти). **`/client/schedule`** (`ClientSchedule`) — **самозапис**: майбутні незаскасовані **group**-заняття (інші типи клієнту не показуємо) вікна «сьогодні+7 днів», згруповані по днях (sticky-заголовок `DOW_LABELS_FULL`+`MONTHS_UK_GENITIVE`). Картка: крапка-колір типу (`typeColor`) + час/тренер/зал/тривалість + **етап хореографії** (тег) + **заповненість** «Місця N/cap · у резерві M» (з `class_availability`) — клієнт бачить різницю занять і повний зал до кліку. Формат — **вертикальний список по днях, НЕ сітка календаря** (свідоме рішення: сітка — антипатерн для mobile-кабінету і дублює адмінський `/schedule`). Кнопка «Записатись» / **«Записатись у резерв»** (бурштинова, коли запис піде в чергу — `goesToWaitlist` за `class_availability`) → **модалка підтвердження** (`ModalShell`, дві секції з ієрархією): **«Заняття»** (назва + рядки коли/тренер/зал `fullWhen` / етап хореографії `choreo_stage` read-only — «не вказано» якщо порожній; тому `choreo_stage` є в `BOOKABLE_CLASS_SELECT`) + **«Запис»** (рядки спишеться «N годин»/залишок стане M для звичайного запису АБО бурштинове інфо-попередження про резерв — НЕ червоне, резерв це не помилка). Підтвердження → `clientEnroll` (RPC `client_enroll`, машинні коди `no_sessions`/`conflict`/`duplicate` → людський текст; **`status` з RPC = `enrolled`\|`waitlist`** → точний бейдж/тост зі словника (`enrollmentStatusLabel`: «Заброньовано»/«У резерві» — без «Ви»-форм). Запити: `listBookableClasses` (filter `ticket_type='group'`) + `getClassAvailability` (числові агрегати заповненості для тексту кнопки; самі enrollments клієнту не видно — `client_select_own`). Вже-записані — бейдж «Заброньовано», без дубля. **⚠️ `client_enroll` НЕ списує сесію при записі** (списання — в `auto_close`), тож БД пропустила б N записів на 1 сесію; UI оптимістично рахує кожен активний запис у вікні зайнятою сесією (`reservedByType`) і блокує кнопку, коли по типу вичерпано (сервер — остання інстанція). **Вартість запису hours-aware** — `sessionCost(duration_min)` (≥120 хв → 2, інакше 1): модалка «спишеться», `reservedByType` і `hasSessions` всі через нього (дзеркало `client_enroll`, який для 2-год ставить `hours_attended=[1,2]`). **`/client/visits`** (`ClientVisits`): майбутні (бейдж-таймер «Через N годин» через `timeUntil`, повна дата `fullWhen` з `MONTHS_UK_GENITIVE`, тренер аватарка+«Тренер», «Після тренування» = наростаючий залишок через **RPC `listMyRunningBalances`/`useAsync`**, НЕ клієнтський цикл) + попередні (бейдж статусу через `enrollmentBadge(client)`, колір tone→`VISIT_TONE_CLASS`); обидва списки — пагінація (`PAGE_SIZE=8` + «Показати ще», клієнтський slice); картка → деталі. **`/client/visits/[id]`** (`VisitDetail`): тренер → дата → дії (Скасувати `client_cancel` лише майбутні + Google-календар) → Заняття (є баланс → «N заняття з абонемента»+залишок; нема → ціна базового тікета `sessions=1`×cost+«буде списано»; cost=`sessions_used`/`hours_attended.length`) → Студія (картка-лінк на `STUDIO.mapsUrl`, без вбудованої карти — щоб не тягнути сторонній трекінг) → Контакти (Telegram/Instagram зі `STUDIO`). Скасування — через `ModalShell` (не `window.confirm`): модалка показує правило відміни з `lib/cancellation.ts` (дедлайн безкоштовності + штраф `cost` годин саме за це заняття). **`/client/subscriptions`** (`ClientSubscriptions`): баланс (депозит акцент + сесії по типах) + історія покупок (sales: тікет=абонемент, `ticket_id=null`=поповнення `+amount_given`/списання `−price_paid`, snapshot інв. #5). Шапка під-екранів — `backHref` (‹ назад). Запити — `client-cabinet-data.ts` (+`getMyEnrollmentDetail` подвійний гейт RLS+client_id, `getBaseTicketPrice`). `CabinetHeader`: props `address`/`backHref`/`hideLogout`. Зона client |
