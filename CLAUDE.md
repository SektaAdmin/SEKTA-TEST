...
| `/client` | **Кабінет клієнта**: депозит, залишки занять по типах, мої майбутні записи + self-відміна (`client_cancel`, з попередженням про списання), **історія покупок** (sales зі snapshot, інв. #5), **рух депозиту** (balance_transactions), **профіль read-only** (імʼя/контакти/депозит + пометка «зверніться до адміністратора»; контакти лише SELECT за RLS — редагувати клієнт не може). Запис на нові заняття — наступна ітерація. Зона client |
