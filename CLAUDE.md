...
| `client_enroll(p_class_id)` | → `(success, enrollment_id, enrolled_status, error_message)`. Клієнтський self-запис із кабінету. Гейт: роль=client, є оплачені заняття типу (`error='no_sessions'`), без конфлікту (`'conflict'`)/дубля (`'duplicate'`), **не в мінус**. **`enrolled_status` = реальний статус ПІСЛЯ тригера `check_class_capacity` (`enrolled`\|`waitlist`)** — `RETURNING id, status` бере значення вже після тригера, щоб кабінет показав точний бейдж/тост (повний зал → «черга»), а не оптимістично «записані». Списання — потім, через auto_close |
