...
| `/client` | **Кабінет клієнта**. Зверху вниз: **профіль read-only** (імʼя/контакти + пометка «зверніться до адміністратора»; контакти лише SELECT за RLS — редагувати клієнт не може) → картки (депозит + залишки занять по типах) → **майбутні записи** + self-відміна (`client_cancel`, з попередженням про списання) → **архів записів** (минулі attended/noshow/cancelled, бейдж статусу) → **покупки** (усі sales: з тікетом = абонемент; `ticket_id=null` = поповнення `+amount_given`/списання `−price_paid` депозиту; snapshot інв. #5, БЕЗ balance_transactions). Запис на нові заняття — наступна ітерація. Зона client |
