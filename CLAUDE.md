...
9. **RLS увімкнено на всіх таблицях, доступ — за роллю через `auth_role()`** (`owner`/`admin`/`trainer`/`client`, дефолт `client`). Модель: owner+admin = повний доступ (`ALL`), trainer/client — вузькі політики (read чужого закрито, write — лише своє). Доменний id у політиках через `current_client_id()`/`current_trainer_id()` (SECURITY DEFINER, мапінг `auth.uid()`→`clients.id`/`trainers.id`). Нова таблиця в міграції → `ENABLE ROW LEVEL SECURITY` + політика `owner_admin_all` (`FOR ALL TO authenticated USING(auth_role() in ('owner','admin')) WITH CHECK(...)`) + потрібні trainer/client-політики + `GRANT SELECT,INSERT,UPDATE,DELETE ... TO anon, authenticated`. RLS-on БЕЗ жодної політики = deny-all (0 рядків без помилки).
...
- **RLS — за роллю через `auth_role()`** (інваріант #9). На ВСІХ доменних таблицях політика `owner_admin_all` (FOR ALL, owner+admin = повний доступ) + потрібні trainer/client-політики; виняток — `class_series` (RLS вимкнено). Матриця доступу (хто бачить чуже vs лише своє) — у [docs/ROLES_PLAN.md](docs/ROLES_PLAN.md) §Фаза 3. Ключове: `client_contacts`/`sales`/`balance_transactions`/`studio_expenses`/`trainer_payments`/`trainer_rates` — trainer не бачить (контакти/гроші); trainer пише `classes`/`enrollments` лише на свої заняття (`trainer_id = current_trainer_id()`); client скрізь бачить лише `*_id = current_client_id()`. ⚠️ RLS-on БЕЗ жодної політики = deny-all (0 рядків без помилки); чистка політик мусить лишати щонайменше `owner_admin_all`. ⚠️ owner/admin поки рівні (ALL) — обмеження admin (без ЗП, без редагування довідників) ще НЕ в RLS, заплановано на Фазу 4.
