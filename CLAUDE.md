...
| `change_enrollment_status(p_enrollment_id, p_new_status, p_force_no_charge=false, p_sessions_used=null)` | → `(success, charged, error_message)`. Єдина точка зміни статусу з UI. Вирівнює баланс сесій + застосовує правило скасування. `charged` = чи списано сесію. **Гейт `can_manage_enrollment()`** (див. нижче) |
...
| `reverse_attendance(p_enrollment_id)` | → `(success, error_message)`. Повертає сесії, status=cancelled, sessions_used=0. **Гейт `can_manage_enrollment()`** |
...
- **Лейбли+класи бейджів** (статуси enrollment, методи оплати, короткі типи, типи balance-транзакцій) → `lib/badges.ts`. `enrollmentStatusClass`/`paymentClass` повертають готовий `'badge badge-cash'` → у `className` напряму. CSS бейджів — у `globals.css`. Лейбли статусів — дієслова (Записалась/Відвідала/Не прийшла/Скасувала/Черга). `personal_card` = «Картка». `transactionTypeLabel(type)` — людська назва `balance_transactions.transaction_type` (purchase/deposit_topup/admin_adjustment + історичні deduction/refund/adjustment).
