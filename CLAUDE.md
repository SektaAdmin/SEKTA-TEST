...
| `/client`, `/client/visits`, `/client/visits/[id]`, `/client/subscriptions` | **Кабінет клієнта** — меню-лаунчер + окремі екрани (стиль Altegio), mobile-first (`layout.tsx` = `.shell` + `.scroll`, бо html/body `overflow:hidden` на моб.). **`/client`** = головна-меню (`ClientHome`): шапка SEKTA + адреса (`STUDIO`) → профіль-картка read-only (аватарка-ініціал + імʼя + email/телефон, «зверніться до адміністрації») → меню (Мої візити / Абонементи / Вийти). **`/client/visits`** (`ClientVisits`): майбутні (бейдж-таймер «Через N годин» через `timeUntil`, повна дата `fullWhen` з `MONTHS_UK_GENITIVE`, тренер аватарка+«Тренер») + попередні (бейдж статусу, колір за `enrollmentStatusClass`); картка → деталі. **`/client/visits/[id]`** (`VisitDetail`): тренер → дата → дії (Скасувати `client_cancel` лише майбутні + Google-календар) → Заняття (є баланс → «N заняття з абонемента»+залишок; нема → ціна базового тікета `sessions=1`×cost+«буде списано»; cost=`sessions_used`/`hours_attended.length`) → Студія (картка-лінк на `STUDIO.mapsUrl`, без вбудованої карти — щоб не тягнути сторонній трекінг) → Контакти (Telegram/Instagram зі `STUDIO`). Скасування — через `ModalShell` (не `window.confirm`): модалка показує правило відміни з `lib/cancellation.ts` (дедлайн безкоштовності + штраф `cost` годин саме за це заняття). **`/client/subscriptions`** (`ClientSubscriptions`): баланс (депозит акцент + сесії по типах) + історія покупок (sales: тікет=абонемент, `ticket_id=null`=поповнення `+amount_given`/списання `−price_paid`, snapshot інв. #5). Шапка під-екранів — `backHref` (‹ назад). Запити — `client-cabinet-data.ts` (+`getMyEnrollmentDetail` подвійний гейт RLS+client_id, `getBaseTicketPrice`). `CabinetHeader`: props `address`/`backHref`/`hideLogout`. Запис на нові заняття — наступна ітерація (`client_enroll` готовий). Зона client |
